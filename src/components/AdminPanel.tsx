...
import { getStoredArticles, saveArticles, BlogArticle, BLOG_CATEGORIES, getValidArticleEyeCatch, mergeWithDefaultArticles } from '../blogData';
import { getStoredSiteContent, saveSiteContent, SiteContent, DEFAULT_SITE_CONTENT } from '../siteContent';
import { getConsultations, updateConsultationStatus, deleteConsultation, saveBlogArticlesToFirestore, saveSiteContentToFirestore, getBlogArticlesFromFirestore, getSiteContentFromFirestore } from '../firebase';
import { 
    ...
    const loadServerData = async () => {
      try {
        ...
        if (firestoreArticles && firestoreArticles.length > 0) {
          const { merged } = mergeWithDefaultArticles(firestoreArticles);
          setArticles(merged);
        } else {
      ...
      try {
        const firestoreSite = await getSiteContentFromFirestore();
        if (firestoreSite) {
          setSiteText(firestoreSite);
        } else {
...
${requestedCategory !== "all" ? `カテゴリーは必ず「${requestedCategory}」にしてください。` : "カテゴリーは 'job', 'salary', 'beginner', 'experienced', 'interview', 'workstyle', 'shops', 'dorm', 'safety', 'beauty', 'faq' の中から適したものを1つ選択してください。"}
${topicPrompt}
...
JSONスキーマ：
{
  ...
  "category": "'job' | 'salary' | 'beginner' | 'experienced' | 'interview' | 'workstyle' | 'shops' | 'dorm' | 'safety' | 'beauty' | 'faq' のいずれか1つ",
  "categoryLabel": "カテゴリーに応じた和名（仕事内容、給料・待遇、未経験、経験者、応募・面接、働き方、お店選び、寮・出稼ぎ、身バレ・安全、その他、FAQ）",
...
4. LINE誘導(cta)ブロックは、記事の中間か最後付近に1つ以上配置してください。ctaブロックは 'type': 'cta' のみで、'text' や 'items' などのキーは不要です。`;
    ...
    const premiumIllustrations = [
      ...
      "/src/assets/images/sakura_advisor_portrait_1788920514818.jpg",
...
export interface BlogArticle {
  ...
  title: string;
  slug: string;
  category: 'job' | 'salary' | 'beginner' | 'experienced' | 'interview' | 'workstyle' | 'shops' | 'dorm' | 'safety' | 'beauty' | 'faq';
...
export const BLOG_CATEGORIES = [
  { id: 'all', label: 'すべて' },
  { id: 'job', label: '仕事内容' },
  { id: 'salary', label: '給料・待遇' },
  { id: 'beginner', label: '未経験' },
  { id: 'experienced', label: '経験者' },
  { id: 'interview', label: '応募・面接' },
  { id: 'workstyle', label: '働き方' },
  { id: 'shops', label: 'お店選び' },
  { id: 'dorm', label: '寮・出稼ぎ' },
  { id: 'safety', label: '身バレ・安全' },
  { id: 'beauty', label: 'その他' },
  { id: 'faq', label: 'FAQ' }
];
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2 bg-surface border border-outline-variant rounded-2xl focus:outline-none focus:ring-1 focus:ring-secondary focus:border-secondary text-sm"
                >
                  <option value="job">仕事内容</option>
                  <option value="salary">給料・待遇</option>
                  <option value="beginner">未経験</option>
                  <option value="experienced">経験者</option>
                  <option value="interview">応募・面接</option>
                  <option value="workstyle">働き方</option>
                  <option value="shops">お店選び</option>
                  <option value="dorm">寮・出稼ぎ</option>
                  <option value="safety">身バレ・安全</option>
                  <option value="beauty">その他</option>
